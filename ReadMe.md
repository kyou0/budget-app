...
### 現在のプロダクト方針
このアプリは、MoneyForward などの家計簿サービスの代替ではなく、**「今月生きていけるか」「借金を返済できるか」「滞納しそうな支払いは何か」** を判断するためのキャッシュフロー制御ツールとして扱う。

重視すること:
- 今月の収入・カード請求・借金返済・固定支出をまとめて、月末に足りるかを即時判定する
- 口座残高の厳密な同期や家計簿明細管理を主目的にしない
- 口座は入出金の紐付け先として扱い、残高は「ざっくり手元資金」として任意入力できればよい
- カレンダーは詳細確認用であり、トップ画面では「不足額」「次に落ちる支払い」「未確定カード」「延滞」を優先表示する
- 旅行・指輪・大型購入など将来の支出予定を入れて、先々の資金不足を事前確認できるようにする
- 毎月の主要操作は「カード請求額を確定する」「臨時収入・追加支出を調整する」「繰上げ返済予定を入れて完済時期と今月不足を確認する」に集約する
- 時給案件・成果報酬などの変動収入は、月次画面で「見込み額」から「今月の実入金額」へ更新し、その月の資金繰り判定と予定データに反映する
- 時給案件は、マスター側で「時給 × 月の想定稼働時間」から月の見込み額を作れるようにし、今月だけの稼働時間・実入金額はダッシュボードで更新する
- マスター設定は、毎月変わらない前提だけを保存する。収入は「月の見込み額・入金予定日・入金先」、固定支出は「毎月の支払額・支払予定日・支払元」、クレジットカードは「カード名・限度額・引落日・引落銀行」を基本情報として扱う
- クレジットカードの「今月いくら落ちるか」はマスターに固定保存せず、ダッシュボードで毎月確定して当月の支払い予定へ反映する
- 日付が確定している今月だけの入金・支払いは、見通し画面ではなくダッシュボードの「今月」に追加し、月末見込みとカレンダー同期対象へ即時反映する
- 税金・保険は、納付書が届いた時点でマスターの税金テンプレートから登録できる。国民健康保険、住民税（普通徴収）、個人事業税、所得税、所得税予定納税、消費税、国民年金などを想定し、期ごとに金額が違うものはダッシュボードの「日付つき収支」で個別入力する
- Google Drive はログイン後にまずクラウド側の同期ファイルを確認し、端末間のデータ反映漏れを避ける。Googleカレンダー同期は変更がない予定をスキップして待ち時間を短縮する
- 銀行口座は残高管理ではなく紐付け先マスターとして扱う。クライアント収入は固定収支の収入一覧へ統合し、カードは引落銀行・引落日などの基本情報だけを持つ
