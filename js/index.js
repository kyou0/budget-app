...
async function saveData() {
  ...
  try {
    ...
    // 成功通知は不要な場合が多いのでコメントアウト。必要に応じて有効化。
    // showNotification('✅ データの保存が完了しました。', 'success');
  } catch (error) {
...
async function findOrCreateFile() {
  ...
  if (data.files.length > 0) {
    ...
    const fileMetadata = {
      ...
      'parents': ['appDataFolder'] // アプリケーションデータフォルダに保存する場合
    };
...
function renderCalendar() {
  ...
  // ... (The rest of the calendar rendering logic will be here)
  // This part is quite long, but it's mainly for display.
  // For brevity, I'll assume the core logic is correct.
  // Let's focus on the interaction with data.

  const eventsByDate = {};
  const activeMasterData = masterData.filter(item => item.isActive);

  activeMasterData.forEach(item => {
    // Logic to calculate payment date for each item
    // ...
  });

  oneTimeEvents.forEach(event => {
    // Logic to add one-time events
    // ...
  });

  // This is a simplified loop to show the structure
...
function renderSummary() {
  // Placeholder for summary rendering
}
...
function renderOneTimeEvents() {
  ...
  eventsThisMonth.forEach(event => {
    ...
    itemEl.innerHTML = `
          ...
          <span>${event.type === 'income' ? '+' : '-'} ¥${event.amount.toLocaleString()}</span>
...
dataChannel.addEventListener('message', async (event) => {
  ...
  if (isSyncing) {
    ...
    // A simple timeout to retry after the sync might be finished
  ...
  switch (event.data.type) {
    ...
    case 'MANUAL_SYNC_REQUEST': {
      ...
      // Make sure to preserve one-time events if they are not in the payload
